...
function Row({
  ...
  async function refresh() {
    ...
    try {
      ...
      setMessage(
        j.matched
          ? `Stats loaded${j.seasonsCounted > 1 ? ` · ${j.seasonsCounted} seasons` : ""}`
          : (j.reason ?? "No stats found")
      );
    } catch (e) {
      setStatus("error");
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted">
          Paste each player&apos;s roster page and their stats follow. Use{" "}
          <b className="text-fg">Find</b> to search for it, paste, press Enter, then{" "}
          <b className="text-fg">↻ Stats</b>.
        </p>
      </div>
      ...
      <div className="card relative z-30 grid grid-cols-1 gap-3 p-4 sm:grid-cols-3">
        ...
        <Select
          value={scope}
          options={["Missing links", "Has a link", "Playing now", "Everyone"]}
          onChange={setScope}
        ...
        {visible.length === 0 ? (
          ...
          <>
            ...
            {visible.slice(0, 100).map((r) => (
              <Row key={r.id} r={r} onSaved={onSaved} />
            ))}
            {visible.length > 100 && (

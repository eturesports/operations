 ...
 *
 * Drawn dark rather than in the NCAA blue, which is what was asked for. That
 * takes some care in two themes: a black roundel disappears on a dark page,
 * so the disc takes `currentColor` — near-black on the light theme, near-white
 * on the dark one — and the lettering is knocked out in whatever sits behind
 * it. In the table that is the surface colour; on a card the pill is always
 * dark, so the disc reads white there and the letters are told to stay dark.
 */
function NcaaMark({ size, knockout }: { size: number; knockout: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 69 69" aria-hidden focusable="false">
      <path d="M59.402 34.312h-7.339l-.878 3.073h1.13l-3.639 5.709-.502-8.782h-7.402l-.815 3.01h1.129l-4.83 7.34h-4.015c-.439 0-2.007-.377-2.007-2.384 0-2.007 1.882-4.893 3.826-4.893h.816c-.125.502-.44 1.506-.44 1.506h3.576l1.255-4.642H33.81c-3.388 0-6.148 2.446-7.026 4.956l1.443-4.956h-4.893l-.878 3.074h1.255l-1.255 4.453-3.2-7.527h-4.578l-.878 3.136h1.254l-2.823 10.288h3.639s1.38-5.081 1.756-6.398a1145.14 1145.14 0 002.634 6.398h4.078l1.882-6.524a8.099 8.099 0 000 3.01c.376 2.134 2.446 3.514 4.704 3.514h7.402l1.255-1.945h5.206l.125 1.882h4.642l1.192-1.882h5.332l.188 1.945h3.262l.69-1.443-.816-11.918z" fill={knockout} />
      <path d="M56.203 47.672l-.188-1.944h-5.332l-1.191 1.882H44.85l-.126-1.882h-5.206l-1.255 1.944h-7.401c-2.259 0-4.391-1.38-4.705-3.512a8.099 8.099 0 010-3.011l-1.882 6.523h-4.077s-2.133-5.206-2.635-6.398c-.313 1.318-1.693 6.398-1.693 6.398h-3.638l2.822-10.287H13.8l.878-3.01h4.579l3.2 7.527 1.254-4.454h-1.255l.878-3.074h4.893l-1.443 4.956c.879-2.51 3.639-4.956 7.026-4.956h5.457l-1.255 4.642h-3.575s.314-1.003.44-1.505h-.816c-1.945 0-3.827 2.885-3.827 4.892 0 2.008 1.631 2.384 2.008 2.384h4.014l4.83-7.34h-1.129l.816-3.01h7.401l.502 8.782 3.638-5.708h-1.129l.878-3.074h7.34l.815 11.918c1.63-3.575 2.509-7.527 2.509-11.73 0-15.619-12.608-28.227-28.227-28.227-15.62 0-28.228 12.608-28.228 28.227 0 15.62 12.609 28.227 28.228 28.227a28.127 28.127 0 0024.965-15.054h-3.262v-.126zM41.65 42.59l2.886-4.328.125 4.265h-3.01v.063zm11.354-.063 2.823-4.265.125 4.265h-2.948z" fill="currentColor" />
    </svg>
  );
}
...
const STYLES = {
  // In the table: quiet until you reach for it. The colour is left to the
  // link, since the two do not want the same one — two `text-` classes on
  // one element would settle it by stylesheet order, which is not a decision.
  row: "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full transition-colors hover:bg-ink-700/60",
  ...
  const links = [
    {
      ...
      // Dark, as asked — and darker than the row's other icon, which stays
      // quiet. `text-fg` is what makes it near-black on the light theme
      // without turning invisible on the dark one.
      className: variant === "row" ? "text-fg" : "",
      icon: <NcaaMark size={size.ncaa} knockout={variant === "card" ? "#14110F" : "rgb(var(--ink-900))"} />,
    },
    {
      key: "instagram",
      href: player.instagramUrl,
      label: `Open ${player.name}'s Instagram`,
      className: variant === "row" ? "text-muted hover:text-fg" : "",
    ...
    <span className={variant === "row" ? "ml-1.5 inline-flex items-center gap-0.5" : "flex items-center gap-1.5"}>
      {links.map((l) => (
        <a
          ...
          title={l.label}
          aria-label={l.label}
          className={`${STYLES[variant]} ${l.className}`.trim()}
        >
          {l.icon}
        </a>
      ))}
